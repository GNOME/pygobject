...
pyg_flags_richcompare (PyGFlags *self, PyObject *other, int op)
{
    ...
    if (PyObject_TypeCheck (other, &PyGFlags_Type)
        && ((PyGFlags *)other)->gtype != self->gtype) {
        g_snprintf (warning, sizeof (warning),
                    "comparing different flags types: %s and %s",
                    g_type_name (self->gtype),
                    g_type_name (((PyGFlags *)other)->gtype));
        if (PyErr_Warn (PyExc_Warning, warning)) return NULL;
    }
...
generate_repr (GType gtype, guint value)
{
    ...
    for (i = 0; i < flags_class->n_values; i++) {
         ...
         */
        if (flags_class->values[i].value == 0) continue;

        if ((value & flags_class->values[i].value)
            == flags_class->values[i].value) {
            if (retval) {
                tmp = g_strdup_printf ("%s | %s", retval,
                                       flags_class->values[i].value_name);
                g_free (retval);
                retval = tmp;
            } else {
                retval =
                    g_strdup_printf ("%s", flags_class->values[i].value_name);
            }
        }
    }
...
pyg_flags_repr (PyGFlags *self)
{
    ...
    tmp = generate_repr (self->gtype,
                         (guint)PyLong_AsUnsignedLongMask ((PyObject *)self));
    ...
    if (tmp)
        retval = g_strdup_printf ("<flags %s of type %s.%s>", tmp, namespace,
...
pyg_flags_from_gtype (GType gtype, guint value)
{
    ...
    if (PyErr_Occurred ()) return PyLong_FromUnsignedLong (0);
               ...
               const char *strip_prefix, GType gtype)
{
    ...
    stub = PyObject_CallFunction ((PyObject *)&PyType_Type, "s(O)O", typename,
                                  (PyObject *)&PyGFlags_Type, instance_dict);
    ...
    if (module) {
        PyDict_SetItemString (
            ((PyTypeObject *)stub)->tp_dict, "__module__",
    ...
    PyDict_SetItemString (((PyTypeObject *)stub)->tp_dict, "__flags_values__",
                          values);
    Py_DECREF (values);
...
pyg_flags_and (PyGFlags *a, PyGFlags *b)
{
    if (!PyGFlags_Check (a) || !PyGFlags_Check (b))
        return PyLong_Type.tp_as_number->nb_and ((PyObject *)a, (PyObject *)b);

    return pyg_flags_from_gtype (
        a->gtype, (guint)(PyLong_AsUnsignedLongMask ((PyObject *)a)
                          & PyLong_AsUnsignedLongMask ((PyObject *)b)));
}
...
{
    if (!PyGFlags_Check (a) || !PyGFlags_Check (b))
        return PyLong_Type.tp_as_number->nb_or ((PyObject *)a, (PyObject *)b);
...
{
    if (!PyGFlags_Check (a) || !PyGFlags_Check (b))
        return PyLong_Type.tp_as_number->nb_xor ((PyObject *)a, (PyObject *)b);
...
pygi_flags_register_types (PyObject *d)
{
    ...
    if (PyType_Ready (&PyGFlags_Type)) return -1;
