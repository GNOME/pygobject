...
def _check_require_version(namespace, stacklevel):
    ...
    warnings.warn(
        f"{namespace} was imported without specifying a version first. "
        f"Use gi.require_version('{namespace}', '{version}') before "
...
class DynamicImporter:
    ...
    def _find_module_check(self, fullname):
        ...
        path, _namespace = fullname.rsplit(".", 1)
    ...
    def create_module(self, spec):
        ...
        if not repository.is_registered(
            namespace
        ) and not repository.enumerate_versions(namespace):
            raise ImportError(
