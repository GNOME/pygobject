                         ...
                         PyGICallableCache  *callable_cache)
{
    ...
    if (!pygi_arg_base_setup ((PyGIArgCache *)seq_cache,
                              type_info,
                              arg_info,
                              transfer,
                              direction)) {
    ...
    seq_cache->arg_cache.destroy_notify = (GDestroyNotify)_sequence_cache_free_func;
    item_type_info = gi_type_info_get_param_type (type_info, 0);
    item_transfer =
        transfer == GI_TRANSFER_CONTAINER ? GI_TRANSFER_NOTHING : transfer;

    seq_cache->item_cache = pygi_arg_cache_new (item_type_info,
                                                NULL,
                                                item_transfer,
                                                direction,
                                                callable_cache,
                          ...
                          PyObject *py_kwnames)
{
     ...
     */
    state->function_ptr = gi_vfunc_info_get_address ((GIVFuncInfo *) vfunc_cache->info,
                                                    implementor_gtype,
                                                    &error);
    if (pygi_error_check (&error)) {
        return FALSE;
