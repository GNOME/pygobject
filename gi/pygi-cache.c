    ...
    GITransfer transfer, PyGIDirection direction)
{
    ...
    if (arg_info != NULL) {
        if (!arg_cache->has_default) {
            ...
            arg_cache->has_default =
                _arg_info_default_value (arg_info, &arg_cache->default_value);
        }
                                          ...
                                          GICallableInfo *callable_info)
{
    ...
    if (callable_cache->py_args == NULL) {
        callable_cache->py_args =
            g_ptr_array_sized_new (callable_cache->n_py_args);
    }
        ...
        if (cancellable && async_callback) {
            ...
            if (container && GI_IS_OBJECT_INFO (container)) {
                ...
                async_finish = gi_repository_find_by_name (
                    NULL, callable_cache->namespace, finish_name);
            } else {
                g_debug (
