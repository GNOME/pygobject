...
{
    if (cache->arg_info && (cache->type_tag == GI_TYPE_TAG_INTERFACE || cache->type_tag == GI_TYPE_TAG_ARRAY))
                                          ...
                                          GICallableInfo *callable_info)
{
    ...
    return_info =
        gi_callable_info_get_return_type (callable_info);
    return_transfer =
        gi_callable_info_get_caller_owns (callable_info);
    return_cache =
    ...
    for (i = 0, arg_index = (guint)callable_cache->args_offset;
        ...
        if (gi_arg_info_get_closure_index (arg_info, &closure_index) && ((gssize) closure_index) == i) {
            ...
            direction = _pygi_get_direction (callable_cache,
                                             gi_arg_info_get_direction (arg_info));
    ...
    if (!pygi_callable_cache_skip_return (callable_cache) && return_cache->type_tag != GI_TYPE_TAG_VOID) {
    ...
    while (arg_cache_item) {
        const gchar *arg_name = pygi_arg_cache_get_name ((PyGIArgCache *)arg_cache_item->data);
                      ...
                      GICallableInfo *callable_info)
{
    ...
    cache->namespace = gi_base_info_get_namespace ((GIBaseInfo *) callable_info);
    ...
    n_args = (gint)cache->args_offset + gi_callable_info_get_n_args (callable_info);
                      ...
                      GICallableInfo *callable_info)
{
    ...
    if (invoker->native_address == NULL) {
        ...
        if (gi_function_invoker_new_for_address (invoker->native_address,
                                                 callable_info,
                                                 invoker,
                                                 &error)) {
            return TRUE;
                                ...
                                PyObject *py_kwnames)
{
    ...
    if (ret == NULL || pygi_callable_cache_skip_return (cache))
        return ret;
                          ...
                          PyObject *py_kwnames)
{
     ...
     */
    state->function_ptr = gi_vfunc_info_get_address (GI_VFUNC_INFO (function_cache->callable_cache.info),
                                                    implementor_gtype,
                                                    &error);
    if (pygi_error_check (&error)) {
        return FALSE;
