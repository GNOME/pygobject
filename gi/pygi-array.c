                             ...
                             gpointer          *cleanup_data)
{
    ...
    if (!PySequence_Check (py_arg)) {
        ...
        return FALSE;
    } else if (PyUnicode_Check (py_arg) && sequence_cache->item_cache->type_tag != GI_TYPE_TAG_UNICHAR) {
        PyErr_SetString (PyExc_TypeError, "Unable to marshal str as an array, use .encode() to convert to bytes");
        return FALSE;
    }
