                             ...
                             gpointer          *cleanup_data)
{
    ...
    if (gi_type_info_get_array_fixed_size (arg_cache->type_info, &fixed_size) && (guint) fixed_size != length) {
    ...
    is_ptr_array = (gi_type_info_get_array_type (arg_cache->type_info) == GI_ARRAY_TYPE_PTR_ARRAY);
    if (is_ptr_array) {
                                     ...
                                     gboolean         was_processed)
{
    if (was_processed) {
        ...
        PyGISequenceCache *sequence_cache = (PyGISequenceCache *)arg_cache;
        PyGIArgGArray *array_cache = (PyGIArgGArray *)arg_cache;
        GIArrayType array_type = gi_type_info_get_array_type (arg_cache->type_info);
               ...
               gpointer           data)
{
    ...
    return g_array_new_take (data, (guint) len, FALSE, (guint)array_cache->item_size);
}
...
static void
pygi_arg_garray_len_arg_setup (PyGIArgGArray *seq_cache,
                               ...
                               gssize *py_arg_index)
{
    ...
    if (!seq_cache->has_len_arg) {
        seq_cache->has_len_arg = gi_type_info_get_array_length_index (type_info, &seq_cache->len_arg_index);
                               ...
                               gssize *py_arg_index)
{
    ...
    pygi_arg_garray_len_arg_setup (array_cache,
                                   type_info,
                                   callable_cache,
                                   direction,
                                   arg_index,
