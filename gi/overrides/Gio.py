...
class DBusConnection(Gio.DBusConnection):
    ...
    def register_object(
        ...
        if method_call_closure is not None:
            ...
            def wrapped_method_call_closure(
                connection,
                sender,
                object_path,
                interface_name,
        ...
        def _process_action(
            ...
            if parameter_type:
                if not GLib.VariantType.string_is_valid(parameter_type):
                    raise TypeError(
                        f"The type string '{parameter_type}' given as the "
                        f"parameter type for action '{name}' is "
            ...
            if state is not None:
                ...
                if change_state is not None:
                    raise ValueError(
                        f"Stateless action '{name}' should give "
                        f"None for 'change_state', not '{change_state}'."
                    )
