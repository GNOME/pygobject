                           ...
                           size_t py_nargsf, PyObject *py_kwnames)
{
    if (self->cache == NULL) {
        ...
        if (GI_IS_FUNCTION_INFO (info)) {
            ...
            if (flags & GI_FUNCTION_IS_CONSTRUCTOR) {
                ...
                function_cache =
                    pygi_method_cache_new (GI_CALLABLE_INFO (info));
            } else {
                function_cache =
                    pygi_function_cache_new (GI_CALLABLE_INFO (info));
            }
    ...
    return pygi_function_cache_invoke (self->cache, py_args, py_nargsf,
                                       py_kwnames);
}
